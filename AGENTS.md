...
A crewmate killed mid-git-operation can leave a git `index.lock` behind that makes the worktree return fail; teardown absorbs that by retrying the return a few times, and it deletes the lock only when the lock is provably abandoned (nothing holds it open and it is old enough), never on age or an unavailable `lsof` alone.
