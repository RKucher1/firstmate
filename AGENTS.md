...
A crewmate killed mid-git-operation can leave a git `index.lock` behind that makes the worktree return fail; teardown absorbs that by retrying the return a few times and then, for a lock still too new to judge, waiting a bounded stretch for it to clear or age, and it deletes the lock only when the lock is provably abandoned on a fresh read (nothing holds it open and it is old enough), never on age, a waited-out timer, or an unavailable `lsof` alone.
When a lock cannot be safely recovered, teardown still clears the window and meta but prints a loud `ERROR:` saying the worktree was NOT returned: that one needs a human to confirm no git process is running against the worktree, clear the lock, and rerun the return - do not treat it as a routine warning.
