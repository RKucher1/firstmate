...
tests/fm-watch-dead-crew.test.sh          # watcher dead-crew reaper: dead pane pids / gone windows reaped (worktree returned, meta cleared), conservative on an unreachable server, live and secondmate crews untouched
...
tests/fm-brief.test.sh                    # brief scaffolding contracts: the no-mistakes DOD shepherd-the-run instruction (F4 strand fix), scout brief stays pipeline-free
...
tests/fm-teardown.test.sh                 # fm-teardown.sh landed-work safety and reminder checks: fork-remote allow, squash/content landings, old-gh/old-git version tolerance (no headRefOid, no merge-tree --write-tree), dirty and unlanded refusals, PR-head metadata, tasks-axi reminder, --force override
tests/fm-teardown-lock-recovery.test.sh   # fm-teardown.sh git index.lock recovery: live-held and unprovable (lsof missing/errored/incomplete) locks are never removed while a routine unrelated lsof mount WARNING still reads as no holder, an invalid staleness threshold cannot skip the age proof, a provably-stale lock is removed and the return retried, a too-young lock is waited out under a stated cap and then re-proved, a failed removal is never logged as one, a transient lock recovers on retry without ever reaching the force-remove branch, and the secondmate-home call site recovers the same way but aborts teardown on refusal
